...
class Solution implements SolutionInterface
{
	...
	private $singles = array('', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine');
	private $tenToTwenty = array('ten', 'eleven', 'twelve', 'thirteen', 'fourteen', 'fifteen', 'sixteen', 'seventeen', 'eighteen', 'nineteen');
	private $tenFolds = array('twenty', 'thirty', 'fourty', 'fifty', 'sixty', 'seventy', 'eighty', 'ninety');
	...
	public function solve() 
	{
		...
		for($i=1; $i<=1000; $i++) 
		{
			...
			{
				$current = $this->singles[floor($i / 100)] . $this->hundred;
				if($i % 100 > 0) {
	...
	/**
	 *	Returns a string representation of any number between 1 and 99
	 *  (1 and 99 included). 
	 *  
	 * @param	i	The integer that is converted
	 * @return string
	**/
	private function belowHundred($i) 
	{
		...
		{
			return $this->tenFolds[floor($i / 10)-2] . $this->singles[$i % 10];
		}
		return null;
	}
}